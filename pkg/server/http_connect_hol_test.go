...
// released reports whether the blocking write has been released. It is a
// non-blocking check.
...
// TestOneOutstandingHTTPDataPacketDoesNotDelayDialResponse pins the
// preparatory writer shell's first-packet handoff behavior.
//
// The test:
...
//
// Once A's sole writer dequeues that packet, the shared backend consumer can
// process B's DIAL_RSP while A's socket Write remains blocked. This is not a
// sustained HOL-isolation proof: a second DATA packet for A blocks on the
// unbuffered writer handoff and can park the shared consumer. The negotiated
// multi-packet contract covers that case separately.
func TestOneOutstandingHTTPDataPacketDoesNotDelayDialResponse(t *testing.T) {
	const (
		agentID    = "agent-1"
	...
	case <-connectionB.connected:
		// B establishes after A's first packet has moved to its sole writer. A
		// must remain blocked and alive; killing A is not the handoff contract.
		if slowHTTP.released() {
			t.Fatal("connection A was released/closed before B established; B must establish via the first-packet handoff, not by killing A")
		}
	case <-time.After(holTestSafetyTimeout):
...
// TestOneOutstandingHTTPDataPacketDoesNotDelayOtherConnectionData pins the
// preparatory writer shell's first-packet DATA handoff between established
// HTTP-CONNECT connections.
//
// The test:
...
//
// B can progress after A's sole writer dequeues one packet. This does not claim
// that DATA dispatch is isolated under sustained traffic: a second DATA packet
// for blocked A waits on the unbuffered handoff and can delay B. The negotiated
// multi-packet contract pins the required final behavior.
func TestOneOutstandingHTTPDataPacketDoesNotDelayOtherConnectionData(t *testing.T) {
	const (
		agentID    = "agent-1"
		connectIDA = int64(1001)
		connectIDB = int64(1002)
	...
	case got := <-recordingHTTP.writes:
		...
		if slowHTTP.released() {
			t.Fatal("connection A was released/closed before B received DATA; B must progress via the first-packet handoff, not by killing A")
		}
	case <-time.After(holTestSafetyTimeout):
...
//  5. Makes A writable, sends another payload, and requires both payloads in
//     byte-stream order.
//  6. Delivers normal CLOSE_RSP and requires close only after A's bytes drain.
//
// This preserves temporary-stall recovery and byte ordering without requiring
// response DATA to be buffered while the legacy connection has no flow-control
// credit contract.
func TestTemporarilySlowHTTPFrontendRecovers(t *testing.T) {
	...
	for {
		...
		if len(gotStream) >= len(payloadsA[0]) {
			break
		}
		select {
		case <-frontendA.streamUpdated:
		case <-deadline.C:
			t.Fatalf("connection A received %d bytes after recovery, want at least %d", len(gotStream), len(payloadsA[0]))
		}
	}
...
//  3. Delivers backend DATA while the response remains blocked.
//  4. Releases the HTTP response, waits for DATA handoff, and completes
//     CLOSE_RSP/CLOSE_REQ shutdown.
...
func TestHTTPConnectResponsePrecedesTunnelData(t *testing.T) {
	...
	// DRAIN is a deterministic marker that the preceding DATA was handed to the
	// sole writer. The unbuffered legacy handoff cannot complete until the writer
	// finishes the CONNECT response.
	...
	case <-feederDone:
	case <-time.After(holTestSafetyTimeout):
		t.Fatal("backend consumer did not hand off DATA after the CONNECT response completed")
	}
	deadline := time.NewTimer(holTestSafetyTimeout)
	defer deadline.Stop()
	for {
