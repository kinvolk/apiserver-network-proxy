...
// frontend. DATA is handed directly to this goroutine without a packet queue.
type httpConnectWriter struct {
	...
	// mu linearizes DATA handoff with graceful and forced terminal transitions.
	// It is never held during HTTP or backend I/O.
	mu                sync.Mutex
	accepting         bool
	pendingData       int
...
func (w *httpConnectWriter) run() {
	...
	for {
		...
		case data := <-w.dataCh:
			...
			// handed-off payload after observing that terminal transition.
			if w.isAborted() {
				return
			}
...
// handoffData transfers one DATA payload directly to the sole writer. It may
// wait for an earlier socket write to finish, but it never stores a packet in a
// connection-local queue.
func (w *httpConnectWriter) handoffData(data []byte) bool {
	w.mu.Lock()
	if !w.accepting {
		w.mu.Unlock()
		return false
	...
	w.mu.Lock()
	w.pendingData--
	if w.closeResponseSeen && w.pendingData == 0 {
		w.gracefulOnce.Do(func() { close(w.gracefulCh) })
	}
	w.mu.Unlock()
	return handedOff
}
...
func (w *httpConnectWriter) abort(reason httpConnectAbortReason) {
	...
	w.frontend.startHTTPForcedCleanup(w.server, reason)
}
...
func (c *ProxyClientConnection) attachHTTPWriter(
	...
	// Only an explicit replacement consumes httpInitialResponse here. A caller
	// attaching nil must either handle a lower-level connection with no
	// configured response or run after configuredHTTPWriter consumed the
	// response before publication. Future claimed-but-unpublished admission
	// paths must abort or attach their fixed terminal response instead of
	// attaching nil before that ownership handoff.
...
func (c *ProxyClientConnection) abortHTTP(server *ProxyServer, reason httpConnectAbortReason) {
	...
	c.startHTTPForcedCleanup(server, reason)
}
