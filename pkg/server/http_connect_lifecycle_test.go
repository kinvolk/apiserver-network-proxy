...
func TestBackendShutdownAbortsOnlyMatchingHTTPConnections(t *testing.T) {
	...
	otherWriter.start()
	activeWriter.handoffData([]byte("active"))
	drainingWriter.handoffData([]byte("draining"))
	otherWriter.handoffData([]byte("other backend"))
...
func TestHTTPConnectGracefulCloseMapLifecycle(t *testing.T) {
	const (
		agentID   = "drain-agent"
		connectID = int64(801)
	)
	...
	writer, _ := connection.attachHTTPWriter(server, nil, false)
	writer.start()
	writer.handoffData([]byte("in flight"))
	select {
	case <-httpWriter.writeStarted:
	case <-time.After(writerTestSafetyTimeout):
		t.Fatal("graceful writer did not enter its blocked write")
	}
	pendingDone := make(chan bool, 1)
	go func() {
		pendingDone <- writer.handoffData([]byte("pending"))
	}()
	writerTestEventually(t, "pending DATA handoff", func() bool {
		writer.mu.Lock()
		defer writer.mu.Unlock()
		return writer.pendingData == 1
	})
	...
	select {
	case handedOff := <-pendingDone:
		if !handedOff {
			t.Fatal("DATA linearized before graceful close was rejected")
		}
	case <-time.After(writerTestSafetyTimeout):
		t.Fatal("pending DATA handoff did not complete")
	}
	stream, closes, _ := httpWriter.snapshot()
	if !bytes.Equal(stream, []byte("in flightpending")) {
		t.Fatalf("gracefully completed stream = %q, want %q", stream, "in flightpending")
	}
	if closes != 1 {
